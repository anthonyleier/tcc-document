...
Para o desenvolvimento do modelo computacional, assim como dos treinamentos e da configuração dos seus recursos, a biblioteca \emph{Tensorflow} havia sido escolhida para atender a essa demanda. O \emph{Tensorflow} possui um grande acervo de funções e utilitários dos mais diversos tipos para se trabalhar com \emph{Machine Learning} e também \emph{Deep Learning}, sendo necessário selecionar quais são os mais adequados a este trabalho. Um dos módulos mais interessantes e que se alinham com o intuito desse trabalho é o \emph{Object Detection}, ou seja, o módulo de detecção de objetos.

O módulo de detecção de objetos do \emph{Tensorflow} é utilizado para encontrar em porções de imagens, elementos que foram pré-treinados para serem reconhecidos. Geralmente é utilizado para detectar pessoas e objetos do dia a dia em imagens. Também pode ser utilizado em vídeos, processando \emph{frame} a \emph{frame}, possibilitando sua aplicação em câmeras de segurança, reconhecendo em tempo real pessoas, veículos, animais e outros elementos que aparecem com bastante frequência.
...
\begin{itemize}
    ...
    \item Número de \emph{Batches}:
    \item Porcentagem de Acerto: A porcentagem de acertos é o número base para ser considerado um acerto de fato, por exemplo, se o algoritmo tem mais de 50\% de certeza para uma célula, é considerado acerto.
\end{itemize}
...
Porcentagem de Acerto & 0.05 & 0.02 & 0.05      \\ \hline
\end{tabular}
	\vspace{6pt}
	\legend{Fonte: Elaborada pelo autor.}
\end{table}
...
Para este trabalho, foi selecionada a ``resnet50'', embora existam muitas outas incluídas na lista da documentação aceitas pelo módulo de detecção de objetos. Essa escolha foi definida pelo tamanho da imagem utilizada e também ao realizar testes de desempenho comparando uma com as outras.
...
\section{Avaliação}
Para a realização da avaliação, foram utilizadas algumas métricas para medir o desempenho e a qualidade dos resultados obtidos.
As métricas foram medidas para cada um dos modelos e configurações diferentes. Até para se ter um comparativo entre uma opção e outra.

As métricas utilizadas, primeiramente, foi a acurácia padrão, onde foi psosivel calcular, o numero de células obtidas em relação ao número real de células.

Outra métrica utilizada, foi o MSE e o rMSE, que ambos significam

Para a realização da comparação entre uma opção e outra, foi realizado

\section{Protótipo}
Após a definição da melhor instância do modelo computacional possível seguindo as métricas definidas, é necessário realizar a exportação desse modelo, para depois utiliza-lo na criação de um protótipo funcional.

Para realizar a exportação do modelo, a biblioteca do Tensorflow precisou ser revista para a criação de Checkpoints. Os checkpoints são uma estrutura presente no Tensorflow que permite salvar o estado atual de um modelo treinado para ser utilizado posteriormente.

Para a criação do protótipo, foi utilizada uma biblioteca extra, o Streamlit. O Streamlit é um framework voltado para o Python, com o intuito de transformar algoritmos de dados em aplicativos web de uma forma fácil e acessível para todos, sem ter a encessidade de se preocupar com o desenvolvimento de um frontend elaborado ou com a estrutura da aplicação, ele permite o foco em apenas reproduzir uma interface gráfica para um modelo computacional em poucos cliques.

Ainda assim é de fácil personalização, demonstrando todos os atibutos que são necessários para o entendimento do processo e também para a interação com o usuário, alterando conforme o input de imagem recebido.
